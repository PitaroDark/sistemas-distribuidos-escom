...
public class Balanceador {
  ...
  public void balanceadorHandler(HttpExchange exchange) throws IOException {
    ...
    CompletableFuture<HttpResponse<String>>[] futures = (CompletableFuture<HttpResponse<String>>[]) new CompletableFuture<?>[SERVIDORES_PORTS.length];
    for (int i = 0; i < SERVIDORES_PORTS.length; i++) {
      int index = i;
      futures[i] = CompletableFuture.supplyAsync(() -> pedirPalabras(SERVIDORES_PORTS[index], nPalabras));
    }
  ...
  public HttpResponse<String> pedirPalabras(String port, int nPalabras) {
    try {
      HttpRequest request = HttpRequest.newBuilder()
          .POST(HttpRequest.BodyPublishers.ofString(nPalabras + ""))
