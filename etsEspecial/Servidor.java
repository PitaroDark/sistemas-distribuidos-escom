...
public class Servidor {
  ...
  public void servidorHandler(HttpExchange exchange) throws IOException {
    ...
    String splitedBody[] = bodyString.split(";");
    int nPalabras = Integer.parseInt(splitedBody[0]);
    int indexLibro = Integer.parseInt(splitedBody[1]);
    ...
    for (int i = indexLibro; i < BOOKS.length; i++) {
      String book = BOOKS[i];
    ...
    String mainBook = BOOKS[indexLibro];
    Set<String> palabrasMainBook = palabrasMap.get(mainBook);
    for (Map.Entry<String, Set<String>> entry : palabrasMap.entrySet()) {
      String nextBook = entry.getKey();
      if (nextBook.equals(mainBook))
        continue;
      Set<String> palabrasNextBook = entry.getValue();
      Set<String> palabrasComunes = new HashSet<>(palabrasMainBook);
      palabrasComunes.retainAll(palabrasNextBook);
      if (palabrasComunes.isEmpty())
        continue;
      comparacionPareja.put(mainBook + " & " + nextBook, palabrasComunes);
    }

    // Parte importante
    StringBuilder responseBuilder = new StringBuilder();

    for (Map.Entry<String, Set<String>> entry : comparacionPareja.entrySet()) {
      String books = entry.getKey();
      Set<String> palabras = entry.getValue();

      StringBuilder palabrasBuilder = new StringBuilder();
      for (String palabra : palabras) {
        if (palabrasBuilder.length() > 0) {
          palabrasBuilder.append(",");
        }
        palabrasBuilder.append(palabra);
      }
      ...
      responseBuilder.append(books).append(":").append(palabrasBuilder);
    }
